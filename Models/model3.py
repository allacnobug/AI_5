...
# 定义整体模型，包括文本模型、图像模型、注意力模块和全连接分类器
class Model(nn.Module):
    ...
    def forward(self, texts, texts_mask, imgs, labels=None):
        ...
        # 将文本特征和图像特征拼接在一起
        combined_feature = torch.cat([text_feature.unsqueeze(0), img_feature.unsqueeze(0)], dim=2)

        # 通过注意力模块进行特征融合
        attention_out = self.attention(combined_feature).squeeze()
